 ...
 *      Changed to PHP Class by Johannes Hof
 *
 */

namespace Hof\PlexExportBundle\Exporter;

class PlexExporter {

    protected $timer_start;
    protected $plex_export_version;
    protected $options;

    public function __construct($defaults) {
        
        $this->timer_start = microtime(true);
        $this->plex_export_version = 1;
        ini_set('memory_limit', '512M');
        //set_error_handler("plex_error_handler");
        //error_reporting(E_ALL ^ E_NOTICE | E_WARNING);

        $this->options = $defaults;
        if (substr($this->options['plex_url'], -1) != '/')
            $this->options['plex_url'] .= '/'; // Always have a trailing slash
        $this->options['absolute_data_dir'] = $this->options['data_dir']; // Run in current dir (PHP CLI defect)
        $this->options['sort_skip_words'] = (array) explode(',', $this->options['sort_skip_words']); # comma separated list of words to skip for sorting titles
    }

    public function run() {
        // Set-up
        $this->plex_log('Welcome to the Plex Exporter v' . $this->plex_export_version);
        $this->check_dependancies(); // Check everything is enabled as necessary
        // Load details about all sections
        $all_sections = $this->load_all_sections();
        if (!$all_sections) {
            $this->plex_error('Could not load section data, aborting');
            exit();
        }
        ...
        // Otherwise, match sections by Title first, then ID
            $sections_to_show = array_filter(explode(',', $this->options['sections']));
            ...
            foreach ($sections_to_show as $section_key_or_title) {
                ...
                $this->plex_error('Could not find section: ' . $section_key_or_title);
            } // end foreach: $sections_to_show
        } // end if: !all sections
        // If no sections found (or matched)
        $num_sections = count($sections);
        if ($num_sections == 0) {
            $this->plex_error('No sections were found to scan');
            exit();
        }
        ...
        foreach ($sections as $i => $section) {
            $this->plex_log('Scanning section: ' . $section['title']);

            $items = $this->load_items_for_section($section);
            if (!$items) {
                $this->plex_error('No items were added for ' . $section['title'] . ', skipping');
            ...
            $this->plex_log('Analysing media items in section...');
            ...
            foreach ($items as $key => $item) {
                ...
                if ($title_first_space > 0) {
                    ...
                    if (in_array($title_first_word, $this->options['sort_skip_words'])) {
                ...
                if (is_array($item['genre']) and count($item['genre']) > 0) {
                    foreach ($item['genre'] as $genre) {
                        if(!isset($raw_section_genres[$genre])){
                            $raw_section_genres[$genre]=0;
                        }
            ...
            $this->plex_log('Added ' . $num_items . ' ' . $this->hl_inflect($num_items, 'item') . ' from the ' . $section['title'] . ' section');
        } // end foreach: $sections_to_export
        
        // Output all data

        $this->plex_log('Exporting data for ' . $num_sections . ' ' . $this->hl_inflect($num_sections, 'section') . ' containing ' . $total_items . ' ' . $this->hl_inflect($total_items, 'item'));

        $output = array(
            'status' => 'success',
            'version' => $this->plex_export_version,
        ...
        $this->plex_log('Generating and minifying JSON output, this may take some time...');
        ...
        $packed_js = $raw_js;
        if (!$packed_js) {
            $this->plex_error('Could not minify JSON output, aborting.');
            exit();
        }

        $filename = $this->options['absolute_data_dir'] . '/data.js';
        $bytes_written = file_put_contents($filename, $packed_js);
        if (!$bytes_written) {
            $this->plex_error('Could not save JSON data to ' . $filename . ', please make sure directory is writeable');
            exit();
        }

        $this->plex_log('Wrote ' . $bytes_written . ' bytes to ' . $filename);

        $timer_end = microtime(true);
        $time_taken = $timer_end - $this->timer_start;
        $this->plex_log('Plex Export completed in ' . round($time_taken, 2) . ' seconds');
    }
     ...
     * Parse a Movie
     * */
    protected function load_data_for_movie($el) {
        ...
        $this->plex_log('Scanning movie: ' . $title);

        $thumb = $this->generate_item_thumbnail(strval($_el->thumb), $key, $title);
        ...
        $url = $this->options['plex_url'] . 'library/metadata/' . $key;
        $xml = $this->load_xml_from_url($url);
        if (!$xml) {
            $this->plex_error('Could not load additional metadata for ' . $title);
            return $item;
        }

        $genres = array();
        foreach ($xml->Video->Genre as $genre)
            $genres[] = strval($genre->attributes()->tag);
        if (count($genres) > 0)
            $item['genre'] = $genres;

        $directors = array();
        foreach ($xml->Video->Director as $director)
            $directors[] = strval($director->attributes()->tag);
        if (count($directors) > 0)
            $item['director'] = $directors;

        $roles = array();
        foreach ($xml->Video->Role as $role)
            $roles[] = strval($role->attributes()->tag);
        if (count($roles) > 0)
            $item['role'] = $roles;
     ...
     * Parse a TV Show
     * */
    protected function load_data_for_show($el) {
        ...
        $this->plex_log('Scanning show: ' . $title);
        ...
        $genres = array();
        foreach ($el->Genre as $genre)
            $genres[] = strval($genre->attributes()->tag);
        if (count($genres) > 0)
            $item['genre'] = $genres;

        $url = $this->options['plex_url'] . 'library/metadata/' . $key . '/children';
        $xml = $this->load_xml_from_url($url);
        if (!$xml) {
            $this->plex_error('Could not load additional metadata for ' . $title);
            return $item;
        ...
        foreach ($xml->Directory as $el2) {
            ...
            $url = $this->options['plex_url'] . 'library/metadata/' . $season_key . '/children';
            $xml2 = $this->load_xml_from_url($url);
            if (!$xml2) {
                $this->plex_error('Could not load season data for ' . $item['title'] . ' : ' . $season['title']);
            }

            $episode_sort_order = array();
            foreach ($xml2->Video as $el3) {
                if ($el3->attributes()->type != 'episode')
                    continue;
     ...
     * */
    protected function load_all_sections() {
        
        $url = $this->options['plex_url'] . 'library/sections';
        $this->plex_log('Searching for sections in the Plex library at ' . $this->options['plex_url']);
        ...
        if ($total_sections <= 0) {
            $this->plex_error('No sections were found in this Plex library');
            return false;
        }
        ...
        foreach ($xml->Directory as $el) {
            ...
            if ($type == 'movie' or $type == 'show') {
                ...
                $this->plex_error('Skipping section of unknown type: ' . $type);
            }
        }

        if ($num_sections == 0) {
            $this->plex_error('No valid sections found, aborting');
            return false;
        }

        if ($total_sections != $num_sections) {
            $this->plex_log('Found ' . $num_sections . ' valid ' . $this->hl_inflect($num_sections, 'section') . ' out of a possible ' . $total_sections . ' ' . hl_inflect($total_sections, 'section') . ' in this Plex library');
        } else {
            $this->plex_log('Found ' . $num_sections . ' ' . $this->hl_inflect($num_sections, 'section') . ' in this Plex library');
        }
     ...
     * */
    protected function load_items_for_section($section) {

        $url = $this->options['plex_url'] . 'library/sections/' . $section['key'] . '/all';
        $xml = $this->load_xml_from_url($url);
        if (!$xml)
            return false;
        ...
        if ($num_items <= 0) {
            $this->plex_error('No items were found in this section, skipping');
            return false;
        }
        ...
        switch ($section['type']) {
            ...
            case 'show':
                ...
                break;
            default:
                $this->plex_error('Unknown section type provided to parse: ' . $section['type']);
                return false;
        }

        $this->plex_log('Found ' . $num_items . ' ' . $this->hl_inflect($num_items, $section['type']) . ' in ' . $section['title']);

        $items = array();
        foreach ($object_to_loop as $el) {
            if($object_parser==='load_data_for_movie'){
               $item = $this->load_data_for_movie($el);   
            }
            if($object_parser==='load_data_for_show'){
               $item = $this->load_data_for_show($el);   
            }
            if ($item)
                $items[$item['key']] = $item;
        }
     ...
     * */
    protected function load_xml_from_url($url) {

        if (!@fopen($url, 'r')) {
            $this->plex_error('The Plex library could not be found at ' . $this->options['plex_url']);
            return false;
        }

        $xml = @simplexml_load_file($url);
        if (!$xml) {
            $this->plex_error('Data could not be read from the Plex server at ' . $url);
            return false;
        }

        if (!$xml) {
            $this->plex_error('Invalid XML returned by the Plex server, aborting');
            return false;
        }
     ...
     * */
    protected function generate_item_thumbnail($thumb_url, $key, $title) {
        ...
        if (!$img_data) {
            $this->plex_error('Could not load thumbnail for ' . $title, ' skipping');
            return false;
        }

        $result = @file_put_contents($save_filename, $img_data);
        if (!$result) {
            $this->plex_error('Could not save thumbnail for ' . $title, ' skipping');
            return false;
        }
     ...
     * */
    protected function plex_log($str) {
        $str = @date('H:i:s') . " $str\n";
        fwrite(STDOUT, $str);
    }
     ...
     * */
    protected function plex_error($str) {
     ...
     * */
    public function plex_error_handler($errno, $errstr, $errfile = null, $errline = null) {
     ...
     * */
    protected function check_dependancies() {
        $errors = false;

        if (!extension_loaded('simplexml')) {
            $this->plex_error('SimpleXML is not enabled');
            $errors = true;
        }

        if (!ini_get('allow_url_fopen')) {
            $this->plex_error('Remote URL access is disabled (allow_url_fopen)');
            $errors = true;
        }

        if (!is_writable($this->options['absolute_data_dir'])) {
            $this->plex_error('Data directory is not writeable at ' . $this->options['absolute_data_dir']);
            $errors = true;
        }

        if ($errors) {
            $this->plex_error('Failed one or more dependancy checks; aborting');
            exit();
        }
    }
     ...
     * */
    protected function hl_parse_arguments($cli_args, $defaults) {
     ...
     * */
    protected function hl_inflect($num, $single, $plural = false) {
        if ($num == 1)
            return $single;
        if ($plural)
            return $plural;
